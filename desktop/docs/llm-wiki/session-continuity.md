...
Pi keeps a **UI journal** (`~/.…/sessions/<appSessionId>/messages.json`) separate from the **Agent session** under `Pi agent home` (`agent-home/sessions/<encoded-cwd>/<agentSessionId>/`).
...
1. Spawn `pi agent stdio` with the same `Pi agent home` / cwd.
...
When Settings flips `session_data_mode` independent↔shared, Host calls `recycle_all_agents` on live + background + parked processes (same kill/soft-disconnect paths as idle recycle). Live `agentSessionId` is cleared so reconnect does not `session/load` against the previous `Pi agent home`. Journals stay. Emits `session://agents_recycled` for a short toast.
...
| Layer | Behavior |
|-------|----------|
| **Agent (Pi)** | **Primary.** Auto-compacts when context ≈ **85%** full (`[session] auto_compact_threshold_percent`). User can also run **`/compact [note]`** in-session. |
