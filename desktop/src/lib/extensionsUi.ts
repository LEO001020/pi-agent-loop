...
export function isCliMissingError(error: string | null | undefined): boolean {
  ...
  return (
    ...
    e.includes("pi build cli not found") ||
 ...
 * (Pi CLI keeps those as distinct concepts).
 */
...
/** Compact meta: scope · version · marketplace/source — mirrors Pi CLI list row. */
...
/**
 * Pi CLI TUI-style provides summary:
...
/** Filter like Pi CLI Plugins tab `f` (all / enabled / disabled). */
...
/**
 * Normalize `pi plugin install <source>` input (path, git URL, owner/repo).
