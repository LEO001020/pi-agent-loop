...
/**
 * A plan review must be resolved through its approve/revise/abandon actions.
 * At every other point the user can return the composer to normal agent mode.
 */
export function canExitPlanMode(input: {
  mode: string;
  planRpcId?: number | null;
}): boolean {
  return input.mode === "plan" && input.planRpcId == null;
}
