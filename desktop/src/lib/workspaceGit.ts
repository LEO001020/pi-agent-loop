...
/**
 * Map repo-relative path (lower-cased, normalised) → coarse kind, for painting
 * U/M badges next to file-tree nodes. Directories are omitted (the tree only
 * marks leaves, matching the IDE reference).
 */
export function buildWorkspaceStatusMap(
  entries: WorkspaceGitFile[],
): Map<string, WorkspaceGitKind> {
  const m = new Map<string, WorkspaceGitKind>();
...
/** True when the index side has staged content (not space / untracked / ignored). */
export function isWorkspaceStaged(entry: WorkspaceGitFile): boolean {
  const idx = entry.indexStatus || " ";
  return idx !== " " && idx !== "?" && idx !== "!";
}

/**
 * Whether discard is offered for this row. Conflicts are blocked; everything
 * else with a known kind can be discarded after an explicit confirm modal.
 */
export function canDiscardWorkspaceEntry(entry: WorkspaceGitFile): boolean {
  if (entry.kind === "conflict" || entry.kind === "ignored") return false;
  if (entry.kind === "unknown") return false;
  return true;
}

/** Split paths for `git_discard` (tracked restore vs untracked clean). */
export function splitDiscardPaths(entry: WorkspaceGitFile): {
  tracked: string[];
  untracked: string[];
} {
  if (entry.kind === "untracked") {
    return { tracked: [], untracked: [entry.path] };
  }
  return { tracked: [entry.path], untracked: [] };
}
 ...
 * Kept for product completeness / narrower discard policies.
 */
