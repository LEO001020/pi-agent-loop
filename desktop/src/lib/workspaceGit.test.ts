...
describe("labels / badge / discard", () => {
  ...
  it("staged detection + discard split + status map", () => {
    const staged: WorkspaceGitFile = {
      path: "a.ts",
      absolutePath: "/p/a.ts",
      status: "M ",
      indexStatus: "M",
    ...
    const untracked: WorkspaceGitFile = {
      path: "n.ts",
      absolutePath: "/p/n.ts",
      status: "??",
      indexStatus: "?",
    ...
    const conflict: WorkspaceGitFile = {
      path: "c.ts",
      absolutePath: "/p/c.ts",
      status: "UU",
      indexStatus: "U",
    ...
    expect(isWorkspaceStaged(staged)).toBe(true);
    expect(isWorkspaceStaged(untracked)).toBe(false);
    expect(canDiscardWorkspaceEntry(staged)).toBe(true);
    expect(canDiscardWorkspaceEntry(untracked)).toBe(true);
    expect(canDiscardWorkspaceEntry(conflict)).toBe(false);
    expect(splitDiscardPaths(staged)).toEqual({
      tracked: ["a.ts"],
      untracked: [],
    });
    expect(splitDiscardPaths(untracked)).toEqual({
      tracked: [],
      untracked: ["n.ts"],
    });
    const map = buildWorkspaceStatusMap([staged, untracked]);
    expect(map.get("a.ts")).toBe("modified");
    expect(map.get("n.ts")).toBe("untracked");
  });
});
